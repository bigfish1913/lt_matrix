...
use ltmatrix::git::{
    ...
    checkout,
    commit::{commit_changes, stage_all, stage_files},
...
fn test_basic_squash_merge() {
    ...
    let initial_commit =
        commit_changes(&repo, "Initial commit").expect("Failed to create initial commit");
    ...
    let feature_commit = commit_changes(&repo, "Add feature").expect("Failed to commit feature");
    ...
    assert_eq!(
        head_commit.id(),
        feature_commit,
        "HEAD should point to feature commit"
    );
    ...
    assert_eq!(
        master_commit.id(),
        initial_commit,
        "Master should still be at initial commit"
    );
    ...
    assert_ne!(
        merged_commit, feature_commit,
        "Squashed commit should have different ID"
    );
    ...
    assert_eq!(
        commit.parent_count(),
        1,
        "Squash merge should have one parent"
    );

    // Verify the file is in the merge commit tree
    let tree = commit.tree().unwrap();
    assert!(
        tree.get_path(std::path::Path::new("src/main.rs")).is_ok(),
...
fn test_squash_merge_multiple_commits() {
    ...
    for i in 1..=3 {
        ...
        commit_changes(&repo, &format!("Add file {}", i)).expect("Failed to commit");
    }
    ...
    for i in 1..=3 {
        ...
        assert!(
            ...
            "File {} should be in merge commit tree",
            i
        );
    }
...
fn test_merge_conflict_detection() {
    ...
    commit_changes(&repo, "Add shared file on master").expect("Failed to commit");

    // Create a feature branch
    create_and_checkout_branch(&repo, "feature-branch").expect("Failed to create feature branch");
    ...
    File::create(&file_path)
        ...
        .unwrap();
    stage_files(&repo, &["shared.txt"]).unwrap();
    commit_changes(&repo, "Modify shared file on feature").expect("Failed to commit");
    ...
    File::create(&file_path)
        ...
        .unwrap();
    stage_files(&repo, &["shared.txt"]).unwrap();
    commit_changes(&repo, "Modify shared file on master").expect("Failed to commit");
    ...
    assert!(
        error_msg.contains("conflict") || error_msg.contains("conflicts"),
...
fn test_merge_no_new_commits_fails() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("no new commits") || error_msg.contains("Nothing to merge"),
...
fn test_merge_with_staged_changes_fails() {
    ...
    commit_changes(&repo, "Add feature").expect("Failed to commit feature");
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("staged"),
        "Error should mention staged changes"
    );
}
...
fn test_merge_staged_changes_error_suggests_actions() {
    ...
    create_and_checkout_branch(&repo, "feature").expect("Failed to create feature branch");
...
fn test_merge_commit_message_formatting() {
    ...
    let merged_commit =
        merge_with_squash(&repo, "feature-branch", message).expect("Failed to merge");
...
fn test_merge_whitespace_message_fails() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("empty") || error_msg.contains("message"),
...
fn test_merge_parent_relationship() {
    ...
    // Merge
    let merged_commit =
        merge_with_squash(&repo, "feature-branch", "Merge feature").expect("Failed to merge");
    ...
    assert_eq!(
        parent.id(),
        head_before.id(),
        "Parent should be HEAD before merge"
    );
}
...
fn test_merge_with_file_additions() {
    ...
    commit_changes(&repo, "Add new file").expect("Failed to commit");
    ...
    // Merge
    let merged_commit =
        merge_with_squash(&repo, "feature-branch", "Merge new file").expect("Failed to merge");
    ...
    assert!(
        tree.get_path(std::path::Path::new("new.txt")).is_ok(),
        "New file should be in commit"
    );
    assert!(
        tree.get_path(std::path::Path::new("existing.txt")).is_ok(),
...
fn test_merge_with_directory_structure() {
    ...
    commit_changes(&repo, "Add component").expect("Failed to commit");
    ...
    // Merge
    let merged_commit =
        merge_with_squash(&repo, "feature-branch", "Merge component").expect("Failed to merge");

    // Verify nested structure exists in commit tree
    let commit = repo.find_commit(merged_commit).unwrap();
    let tree = commit.tree().unwrap();
    assert!(
        tree.get_path(std::path::Path::new("src/components/component.rs"))
            .is_ok(),
