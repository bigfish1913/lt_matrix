...
mod ci_workflow_tests {
    ...
    fn ci_workflow_comprehensive() {
        ...
        assert!(
            workflow.is_mapping(),
            "CI workflow should be a valid YAML mapping"
        );
        assert_eq!(
        ...
        let push_branches = push
            .get("branches")
            .expect("Push trigger should have 'branches'");
        ...
        let pr = on
            .get("pull_request")
            .expect("Workflow should have 'pull_request' trigger");
        let pr_branches = pr
            .get("branches")
            .expect("PR trigger should have 'branches'");
    ...
    fn ci_clippy_job_valid() {
        ...
        let steps = clippy_job
            .get("steps")
            .expect("clippy job should have steps");
        ...
        assert!(
            has_clippy,
            "clippy job should run 'cargo clippy --all-targets -- -D warnings'"
        );
    }
    ...
    fn ci_tests_jobs_valid() {
        ...
        let unit_steps = unit_job
            .get("steps")
            .expect("test-unit job should have steps");
        ...
        assert!(
            has_unit_test,
            "unit tests job should run 'cargo test --lib'"
        );

        // Integration tests
        let int_job = jobs
            .get("test-integration")
            .expect("Should have 'test-integration' job");
        let int_steps = int_job
            .get("steps")
            .expect("test-integration job should have steps");
    ...
    fn ci_has_caching() {
        ...
        let jobs_with_cache: usize = jobs
            .as_mapping()
            .unwrap()
            .values()
            .filter(|job| {
...
mod release_workflow_tests {
    ...
    fn release_workflow_comprehensive() {
        ...
        assert!(
            workflow.is_mapping(),
            "Release workflow should be a valid YAML mapping"
        );
        assert_eq!(
            workflow.get("name").and_then(|v| v.as_str()),
            Some("Release"),
        ...
        assert!(
            job_names
                .iter()
                .any(|j| j.contains("release") && j.contains("create")),
    ...
    fn release_has_caching() {
        ...
        let build_jobs_with_cache: usize = jobs
            .as_mapping()
            .unwrap()
            .iter()
            .filter(|(k, _)| k.as_str().map(|s| s.contains("build")).unwrap_or(false))
        ...
        if let Some(job) = linux_build_job {
            ...
            let has_zigbuild_install = steps.iter().any(|step| {
                step.get("run")
                    .and_then(|r| r.as_str())
                    .map(|r| {
                        r.contains("cargo-zigbuild") || r.contains("pip install cargo-zigbuild")
                    })
                    .unwrap_or(false)
            });
    ...
    fn release_supports_draft_releases() {
        ...
        assert!(
            draft_input.is_some(),
            "workflow_dispatch should have 'draft' input"
        );
    ...
    fn release_builds_all_target_platforms() {
        ...
        if let Some(job) = linux_job {
            ...
            if let Some(matrix_includes) = matrix.and_then(|m| m.as_sequence()) {
                let targets: Vec<&str> = matrix_includes
                    .iter()
        ...
        assert!(
            macos_universal.is_some(),
            "Should have macOS universal binary build job"
        );
    ...
    fn release_creates_github_release_with_artifacts() {
        ...
        assert!(
            release_step.is_some(),
            "create-release should use a release action"
        );
        ...
        if let Some(step) = release_step {
            ...
            assert!(
                with.is_some(),
                "Release action should have 'with' configuration"
            );
    ...
    fn release_triggers_on_version_tags() {
        ...
        assert!(
            tag_patterns
                .iter()
                .any(|p| p.starts_with('v') && p.contains('*')),
    ...
    fn release_has_prepare_job() {
        ...
        assert!(
            prepare.is_some(),
            "Release workflow should have 'prepare' job"
        );
    ...
    fn release_has_release_summary() {
        ...
        let summary_job = jobs
            .as_mapping()
            .unwrap()
            .iter()
            .find(|(k, _)| k.as_str().map(|s| s.contains("summary")).unwrap_or(false));

        assert!(
            summary_job.is_some(),
            "Release workflow should have release summary job"
        );
    }
}
...
mod best_practices_tests {
    ...
    fn ci_status_aggregation() {
        ...
        let status_job = jobs.as_mapping().unwrap().iter().find(|(k, _)| {
            k.as_str()
        ...
        if let Some((_, job)) = status_job {
            ...
            assert!(needs.is_some(), "Status job should have dependencies");
        }
    }
}
