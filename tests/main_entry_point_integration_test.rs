...
fn test_logging_initialization_with_file() {
    ...
    assert!(
        result.is_ok(),
        "Logging initialization should succeed with valid file path"
    );
...
fn test_logging_initialization_console_only() {
    ...
    assert!(
        result.is_ok(),
        "Console-only logging initialization should succeed"
    );
}
...
fn test_cli_overrides_creation() {
    ...
    let args = Args::parse_from([
        // No .expect() needed for parse_from
        "ltmatrix",
        "test goal",
        "--fast",
        "--agent",
...
fn test_agent_backend_support_check() {
    ...
    assert!(
        !factory.is_supported("nonexistent"),
        "nonexistent backend should not be supported"
    );
}
...
fn test_error_chain_display() {
    ...
    assert!(
        error_string.contains("Base error occurred") || error_string.contains("Additional context"),
...
fn test_permission_error_hint() {
    ...
    assert!(
        ...
        "Test error should contain permission"
    );
...
fn test_network_error_hint() {
    ...
    assert!(
        ...
        "Test error should contain network"
    );
...
fn test_config_error_hint() {
    ...
    assert!(
        lower_msg.contains("config"),
        "Test error should contain config"
    );
...
fn test_agent_error_hint() {
    ...
    assert!(
        lower_msg.contains("agent"),
        "Test error should contain agent"
    );
    ...
    if lower_msg.contains("agent") || lower_msg.contains("backend") {
        ...
        assert!(
            hint.contains("agent") || hint.contains("backend"),
...
fn test_shutdown_flag_functionality() {
    ...
    assert!(
        !TEST_SHUTDOWN_FLAG.load(Ordering::SeqCst),
...
fn test_full_application_flow() {
    ...
    assert_eq!(
        ...
        ltmatrix::cli::args::ExecutionModeArg::Fast
    );
