...
**规范文件位置**：
```
.ltmatrix/guidelines/            # 按 Agent 类型分文件（推荐）
...
# 或单文件方式（按章节区分）
.ltmatrix/guidelines.md          # 包含所有规范的单一文件
```

**规范文件示例**：

**`.ltmatrix/guidelines/_common.md`**（通用规范）：
```markdown
# 通用规范
...
**`.ltmatrix/guidelines/dev.md`**（开发规范）：
```markdown
# 开发规范
...
**`.ltmatrix/guidelines/test.md`**（测试规范）：
```markdown
# 测试规范
...
**`.ltmatrix/guidelines/review.md`**（审查规范）：
```markdown
# 审查规范
...
**`.ltmatrix/guidelines/plan.md`**（规划规范）：
```markdown
# 规划规范
...
```toml
[project]
# 规范目录（可选，默认 .ltmatrix/guidelines/）
guidelines_dir = ".ltmatrix/guidelines"
...
- **项目级记忆**: `.ltmatrix/memory/project.json`
  - 项目结构
  - 技术栈
  - 编码规范
  - 已完成任务摘要
...
- **全局记忆**: `~/.ltmatrix/memory/global.json`
  - 通用最佳实践
  - 常见问题解决方案
- **运行记忆**: `.ltmatrix/memory/run-{session-id}.json`
  - 本次运行的上下文
  - Agent 会话记录
